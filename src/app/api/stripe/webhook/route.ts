...
import { stripe } from '@/lib/stripe/stripe-action'
import { subscriptionCreated } from '@/lib/stripe'
...
export async function POST(req: NextRequest) {
  ...
  const headersInstance = await headers(); // Await the promise
  const sig = headersInstance.get('Stripe-Signature')  
  const webhookSecret =
