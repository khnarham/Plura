...
import { changeUserPermission, getAuthUserDetails, getUserPermissions, saveActivityLogsNotification, updateUser } from '@/lib/quries'
import { z } from 'zod'
...
import Loading from '@/app/site/components/forms/loading'
...
const UserDetails = ({id, type , userData , subAccounts}: Props) => {
  ...
  useEffect(()=>{
    ...
    const fetchPermissions = async() =>{
        ...
        if(permissions) setSubAccountsPermissions(permissions)
    }
        fetchPermissions()
  }, [data , form])
...
const onSubmit = async(values: z.infer<typeof userDataSchema>) =>{
    ...
    if(userData || data?.user){
        const updatedUser = await updateUser(values);
         authUserData?.Agency?.SubAccount.filter((subac)=>{
            authUserData?.Permissions.find((p) => p.subAccountId === subac.id && p.access)
         }).forEach(async(subAccounts)=>{
             await saveActivityLogsNotification({
                agencyId: undefined,
                description: `Updated ${userData?.name}`,
